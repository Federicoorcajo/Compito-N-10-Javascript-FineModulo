...
{
  result: [], <-- inserisci qui le inserzioni che rispecchiano la posizione lavorativa e 
  la posizione geografica richiesta
...
const cercaJob = function (title, location) {
  let cercaJobs = [];
  for (let i = 0; i < jobs.length; i++) {
    if (
      ...
      cercaJobs.push(jobs[i]);
    }
  }
  return { result: cercaJobs, count: cercaJobs.length };
};

//COLLEGAMENTO CON BUTTON

function cercaEVisualizza() {
  const jobInput = document.getElementById("jobTitle").value.toLowerCase();
  const locationInput = document.getElementById("location").value.toLowerCase();
  const { result, count } = cercaJob(jobInput, locationInput);

  const risultatiUl = document.getElementById("risultati");
  risultatiUl.innerHTML = ""; // Pulisce eventuali risultati precedenti

  if (count === 0) {
    risultatiUl.innerHTML = "<li>Nessun risultato trovato</li>";
  } else {
    result.forEach((cercaJob) => {
      const listItem = document.createElement("li");
      listItem.textContent = `Titolo: ${cercaJob.title}, Posizione: ${cercaJob.location}`;
      risultatiUl.appendChild(listItem);
    });
  }
}
